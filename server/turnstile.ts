...
export async function verifyTurnstileToken(
  ...
  if (!secret) return { ok: false, errorCodes: ['secret-absent-de-l-environnement'] };
  if (!token) return { ok: false, errorCodes: ['jeton-absent-du-formulaire'] };
  ...
  try {
    ...
    return { ok: false, errorCodes: ['siteverify-injoignable'] };
  }
  ...
  try {
    ...
    return { ok: false, errorCodes: ['siteverify-reponse-illisible'] };
  }
