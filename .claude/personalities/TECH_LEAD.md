...
1. **Launch an independent R1 REVIEWER** with **`bin/autonomous_reviewer "<ID> (PR #<num>)"`** in the background, or `bin/reviewer "<ID> (PR #<num>)"` for an attended round. Never a paraphrase of the personality, never a generic agent, never a review inside your own session. Those wrappers seed `REVIEWER.md` verbatim and append the stable procedure from `.claude/reviewer-append.txt`, so the call carries only `<ID> (PR #<num>)`. **Invoke the copy that lives in this item's own worktree** (`.claude/worktrees/<slug>/bin/autonomous_reviewer "..."` or `.claude/worktrees/<slug>/bin/reviewer "..."`), never the main checkout's: both scripts read `REVIEWER.md` from wherever they are run, and the main checkout drifts the moment nobody pulls it, which seeds the round with a stale personality regardless of how current the review worktree it creates in step 3 of `review.md` turns out to be. The autonomous variant runs unattended through a targeted permission allowlist (`.claude/autonomous-reviewer-settings.json`), never `--dangerously-skip-permissions`. If the environment blocks it, STOP and hand the founder the ready-to-paste command.
