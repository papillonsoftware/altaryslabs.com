...
2. If you are on `main` or on `post-refonte`, or in the main checkout rather than inside `.claude/worktrees/`, STOP and warn the user: "This branch receives no direct commits. Create a worktree first: `git worktree add .claude/worktrees/<slug> -b <type>/<slug> origin/post-refonte`."
...
9. Create a PR **targeting `post-refonte`**, never `main`. `main` is the live production branch since the 2026-08-08 DNS cutover: merging into it deploys immediately.
