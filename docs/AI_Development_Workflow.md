...
- **Both launchers refuse to start when the worktree they run from is behind `origin/refonte-multipages`,** since that is the tree they read the reviewer's instructions out of. The convention above says which copy to invoke; this guard, in `bin/lib/fraicheur.sh`, checks that the copy is actually current. It was added after a round was seeded with instructions that D033 and D037 had already annulled, without the round having any way to notice. Update the item branch from the integration branch, then relaunch; do not work around it. See D064.
