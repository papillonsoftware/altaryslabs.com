...
- **`post-refonte` is the integration branch and receives no direct commits.** Every work item lives in its own branch or worktree and lands through a PR targeting `post-refonte`. It was named `refonte-multipages` until the rebuild shipped; renamed 2026-08-08 once its purpose became carrying post-launch follow-ups rather than the rebuild itself. See D100.
- **Never work in the main checkout.** Every work item gets its own worktree: `git worktree add -b <type>/<slug> .claude/worktrees/<slug> post-refonte`
- **Never commit on `main`, and never target `main` in a PR.**
- **`main` is the live production branch since the 2026-08-08 DNS cutover**, deployed automatically by Cloudflare Pages on every push. There is no GitHub Pages safety net behind it any more: merging `post-refonte` into `main` reaches `altaryslabs.com` immediately. That merge stays a founder decision, made deliberately each time, for that reason now rather than the pre-cutover one.
